...
function MoreScreen({ bottomInset, role, onSignOut }: { bottomInset: number; role: Role; onSignOut: () => void }) {
  ...
  const expenseTypes = useQuery(api.money.listExpenseTypes, canSeeMoney ? {} : "skip");
  const expenseTypeBreakdown = useQuery(api.money.expensesByType, canSeeMoney ? reportBounds : "skip");
  const addExpense = useMutation(api.money.addExpense);
  const addExpenseType = useMutation(api.money.addExpenseType);
  const removeExpenseType = useMutation(api.money.removeExpenseType);
  ...
  const updateOpeningCash = useMutation(api.shifts.updateOpeningCash);
  ...
  const [expenseTypeSaving, setExpenseTypeSaving] = useState(false);
  ...
  const saveExpense = async () => {
    ...
    if (!expenseType) { setError("Pick a type for this expense."); return; }
    setSaving(true); setError(null);
    try { await addExpense({ description, amountKobo, paymentMethod: expenseMethod, type: expenseType }); setExpenseOpen(false); setDescription(""); setExpenseAmount(""); setExpenseMethod("cash"); setExpenseType(undefined); } catch (saveError) { setError(saveError instanceof Error ? saveError.message : "Could not save expense."); } finally { setSaving(false); }
  };
  const saveNewExpenseType = async () => {
    const name = newExpenseType.trim();
    if (!name) { setError("Enter a name for the new expense type."); return; }
    setExpenseTypeSaving(true); setError(null);
    try { await addExpenseType({ name }); setNewExpenseType(""); setExpenseType(name); } catch (saveError) { setError(saveError instanceof Error ? saveError.message : "Could not add the type."); } finally { setExpenseTypeSaving(false); }
  };
  const deleteExpenseType = async (id: Id<"expenseTypes">) => {
    setError(null);
    try { await removeExpenseType({ id }); } catch (saveError) { setError(saveError instanceof Error ? saveError.message : "Could not remove the type."); }
  };
  ...
  const saveOpeningCash = async () => {
    ...
    const cashKobo = nairaInputToKobo(openingCashDraft);
    if (cashKobo < 0) { setError("Enter a valid cash amount."); return; }
    setSaving(true); setError(null);
    try {
      await updateOpeningCash({ openingCashKobo: cashKobo });
  ...
  return (
    <View style={styles.moreScreen}>
      ...
      <View style={styles.moreHeader}><View style={styles.headerBrandRow}><BrandMark size={30} /><Text style={styles.moreTitle}>Money & more</Text></View><Pressable style={styles.moreIcon} onPress={() => setRoleModalOpen(true)}><Ionicons name="person-circle-outline" size={26} color="#A34A30" /></Pressable></View>
        ...
        <View style={styles.moreSectionHeader}><Text style={styles.moreSectionTitle}>Expenses by type</Text><Text style={styles.moreSectionHint}>{reportPeriod === "today" ? "Today" : reportPeriod === "week" ? "7 days" : "30 days"}</Text></View>
        {expenseTypeBreakdown?.length ? <View style={styles.moneyList}>{expenseTypeBreakdown.slice(0, 6).map((group) => <View key={group.type} style={styles.moneyListRow}><View style={[styles.moneyListIcon, { backgroundColor: "#F7E4DC" }]}><Ionicons name="pricetag-outline" size={18} color="#A34A30" /></View><View style={styles.moneyListMain}><Text style={styles.moneyListTitle}>{group.type}</Text><Text style={styles.moneyListMeta}>{group.count} {group.count === 1 ? "expense" : "expenses"}</Text></View><Text style={styles.moneyListAmount}>{formatNaira(group.totalKobo)}</Text></View>)}</View> : <View style={styles.moreEmpty}><Text style={styles.moreEmptyText}>No expenses in this period.</Text></View>}
        <View style={styles.moreSectionHeader}><Text style={styles.moreSectionTitle}>Expense types</Text><Text style={styles.moreSectionHint}>Categories for spending</Text></View>
        <View style={styles.moneyList}>{expenseTypes?.length ? expenseTypes.map((type) => <View key={type._id} style={styles.moneyListRow}><View style={styles.moneyListIcon}><Ionicons name="pricetag" size={17} color="#A34A30" /></View><View style={styles.moneyListMain}><Text style={styles.moneyListTitle}>{type.name}</Text></View><Pressable onPress={() => deleteExpenseType(type._id)} hitSlop={8}><Ionicons name="trash-outline" size={18} color="#A34A30" /></Pressable></View>) : <View style={styles.moreEmpty}><Text style={styles.moreEmptyText}>No expense types yet.</Text></View>}<View style={styles.expenseTypeAddRow}><View style={[styles.reasonInputWrap, styles.expenseTypeInputWrap]}><TextInput value={newExpenseType} onChangeText={(value) => { setNewExpenseType(value); setError(null); }} placeholder="Add type e.g. Transport" placeholderTextColor="#A0A49D" style={styles.reasonInput} /></View><Pressable style={[styles.lineSaveButton, expenseTypeSaving && styles.sendButtonDisabled]} onPress={saveNewExpenseType} disabled={expenseTypeSaving}>{expenseTypeSaving ? <ActivityIndicator color="#FFFFFF" size="small" /> : <Text style={styles.lineSaveText}>Add</Text>}</Pressable></View></View>
...
const styles = StyleSheet.create({
  ...
  editOpeningInline: { flexDirection: "row", alignItems: "center", gap: 8 },
  editOpeningInput: { backgroundColor: "#FFFFFF", borderWidth: 1, borderColor: "#E0E2DB", borderRadius: 12, paddingHorizontal: 12, paddingVertical: 7, minWidth: 120, color: "#3B322C", fontSize: 14, fontWeight: "700" },
  editOpeningValue: { flexDirection: "row", alignItems: "center", gap: 8 },
  ...
  expenseTypeChips: { flexDirection: "row", flexWrap: "wrap", gap: 8, marginBottom: 14 },
  expenseTypeChip: { borderRadius: 14, backgroundColor: "#EFE7DF", paddingHorizontal: 14, paddingVertical: 10 },
  expenseTypeChipActive: { backgroundColor: "#A84629" },
  expenseTypeChipText: { color: "#756A62", fontSize: 13, fontWeight: "800" },
  expenseTypeChipTextActive: { color: "#FFFFFF" },
  expenseTypeAddRow: { flexDirection: "row", gap: 10, alignItems: "center", marginBottom: 14 },
  expenseTypeInputWrap: { flex: 1 },
